...
void ConnectionReport::dump(ceph::Formatter *f) const
{
  ...
  f->open_object_section("peer scores");
  for (auto i : history) {
    f->open_object_section("peer");
    f->dump_int("peer rank", i.first);
    f->dump_float("peer score", i.second);
    f->dump_bool("peer alive", current.find(i.first)->second);
    f->close_section();
  }
...
void ConnectionTracker::dump(ceph::Formatter *f) const
{
  ...
  f->dump_float("half life", half_life);
